(() => {
  ...
  function setupCanvas(){
    ...
    if(els.inputDebug) els.inputDebug.textContent = "V10 / 入力待機";
    ...
    const begin = e => {
      ...
      if(els.inputDebug) els.inputDebug.textContent = `V10 input: ${kind}`;
    };
    ...
    // リサイズ時は保持しているstrokeを再描画
    window.addEventListener("resize",()=>{
      const r = handCanvas.getBoundingClientRect();
      const newDpr = window.devicePixelRatio || 1;
      handCanvas.width = Math.max(1,Math.round(r.width*newDpr));
      handCanvas.height = Math.max(1,Math.round(r.height*newDpr));
      ctx = handCanvas.getContext("2d",{alpha:true,desynchronized:true});
